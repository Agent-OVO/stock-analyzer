...
from src.news_service.factory import get_news_provider
...
class HistoryService:
    ...
    def get_news_intel(self, query_id: str, limit: int = 20) -> List[Dict[str, str]]:
        ...
        try:
            ...
            # 如果仍然没有找到新闻，尝试使用 NewsAggregator 实时获取
            if not records:
    ...
    def _fetch_news_from_aggregator(self, query_id: str, limit: int) -> List[Any]:
        """
        Fetch news in real-time using specialized stock news provider when database has no stored news.

        优先使用专用个股新闻源（StockNewsProvider），包含：
        1. 东方财富个股新闻
        2. 巨潮资讯官方公告
        ...
        Returns:
            List of news records compatible with database schema
        """
        try:
            # Get stock code and name from analysis history
            ...
            # 优先使用专用个股新闻提供者（StockNewsProvider）
            try:
                from src.news_service.factory import NewsProviderFactory

                # 尝试获取专用个股新闻提供者
                try:
                    stock_news_provider = NewsProviderFactory.create('stock_news')
                    if stock_news_provider and stock_news_provider.is_available():
                        news_items = stock_news_provider.search_stock_news(
                        ...
                        if news_items:
                            # Convert NewsItem to database-like records
                            from collections import namedtuple
                            NewsRecord = namedtuple('NewsRecord', ['title', 'snippet', 'url', 'fetched_at'])

                            result_records = []
                            for item in news_items:
                                snippet = (item.content or "")[:500] if item.content else ""
                                # 添加来源标签
                                source_tag = f"[{item.source}]"
                                result_records.append(NewsRecord(
                                    title=f"{source_tag} {item.title}" if item.source else item.title,
                                    snippet=snippet,
                                    url=item.url,
                                    fetched_at=datetime.now()
                                ))

                            logger.info(f"从 StockNewsProvider 获取到 {len(result_records)} 条个股资讯 for {stock_code}")
                            return result_records
                except Exception as e:
                    logger.debug(f"StockNewsProvider 不可用: {e}")
                ...
                if news_provider and news_provider.is_available():
                    ...
                    if news_items:
                        ...
                        logger.info(f"从通用新闻提供者获取到 {len(result_records)} 条股票新闻 for {stock_code}")
                        return result_records
                else:
                    logger.debug("所有新闻提供者均不可用")
            except Exception as e:
